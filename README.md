...
### 2026.02.15-28
* I wrote this file online on Git and messed up the merge, so I lost a little progress from the development logs, I will try to rewrite it from memory.
* I created the Header.h file to contain the headers.
* Tried to send out a frame, despite receiving error code 0, it was not received on my machine. After a long debugging session, I found several issues:
    * The TX descriptors did not start at the right memory address. In CubeMX, I went with the default 4 RX and 4 TX descriptors. The RX one should start at 0x30000000 and the TX one should start at 0x30000080, but since I used the same section for the buffers and the descriptors, they didn't come after each other. I had to hardcode the starting addresses (After this even though nothing seemed to change in debug mode, RX stopped working entirely. It didn't start working again until I removed the padding I had added earlier, which I only added because it wasn't working without it in the first place.) and create a separate section for everything, because even after I separated them, the end of the descriptor memory space was overlapped by the start of the TX buffer address.
